...
class BitfinexAuthenticatedAccountAction
{
     ...
     * @throws BitfinexPathNotFoundException
     */
    final public function movements(string $currency): BitfinexResponse
    {
        $request = (new BitfinexRequest($this->request, $this->credentials, $this->client));

        $response = $request->execute(apiPath: $this->url->setPath("$this->basePath.movements", ['currency' => $currency])->getPath());
    ...
    final public function movementInfo(string|int $id): BitfinexResponse
    {
        ...
        $response = $request->execute(apiPath: $this->url->setPath("$this->basePath.movement_info")->getPath());
