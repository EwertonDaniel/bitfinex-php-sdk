...
- A rejected write no longer reaches the caller as a success. `STATUS` is documented as an open set, so anything other than `SUCCESS` fails, and the reason in `TEXT` is no longer dropped.
